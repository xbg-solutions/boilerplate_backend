...
export function initializeAddressValidator(config: { apiKey: string }): void {
  mapsConfig = config;
}

function isGoogleMapsConfigured(): boolean {
  return mapsConfig != null && !!mapsConfig.apiKey;
}

function getGoogleMapsApiKey(): string {
  if (!mapsConfig || !mapsConfig.apiKey) {
    throw new Error('Google Maps API key not configured. Call initializeAddressValidator() first.');
  }
