 ...
 * Path pattern: {{#each ancestorChain}}{{collectionName}}/{{{paramName}}}/{{/each}}{{collectionName}}
 */
...
{{#each relationships}}
{{#if targetStorage}}
{{#ifEquals type 'one-to-many'}}
{{#ifEquals targetStorage.type 'subcollection'}}
  /**
   * Get {{name}} for {{../entityName}} (subcollection query)
   */
  async get{{capitalize name}}({{#each targetStorage.ancestorChain}}{{paramName}}: string, {{/each}}): Promise<{{entity}}[]> {
    let ref: any = this.db;
{{#each targetStorage.ancestorChain}}
    ref = ref.collection('{{collectionName}}').doc({{paramName}});
{{/each}}
    const snapshot = await ref.collection('{{targetStorage.collectionName}}').get();
    return snapshot.docs.map((doc: any) => {{entity}}.fromFirestore(doc.id, doc.data()));
  }
{{else}}
  /**
   * Get {{name}} for {{../entityName}} (foreign key query)
   */
  async get{{capitalize name}}({{../entityNameLower}}Id: string): Promise<{{entity}}[]> {
    const snapshot = await this.db
      .collection('{{targetStorage.collectionName}}')
      .where('{{foreignKey}}', '==', {{../entityNameLower}}Id)
      .get();
    return snapshot.docs.map((doc: any) => {{entity}}.fromFirestore(doc.id, doc.data()));
  }
{{/ifEquals}}
{{/ifEquals}}
{{#ifEquals type 'many-to-one'}}
  /**
   * Get {{name}} for {{../entityName}} (single lookup)
   */
  async get{{capitalize name}}({{foreignKey}}: string): Promise<{{entity}} | null> {
    const doc = await this.db.collection('{{targetStorage.collectionName}}').doc({{foreignKey}}).get();
    return doc.exists ? {{entity}}.fromFirestore(doc.id, doc.data()!) : null;
  }
{{/ifEquals}}
{{#ifEquals type 'one-to-one'}}
{{#ifEquals targetStorage.type 'subcollection'}}
  /**
   * Get {{name}} for {{../entityName}} (subcollection singleton)
   */
  async get{{capitalize name}}({{#each targetStorage.ancestorChain}}{{paramName}}: string, {{/each}}): Promise<{{entity}} | null> {
    let ref: any = this.db;
{{#each targetStorage.ancestorChain}}
    ref = ref.collection('{{collectionName}}').doc({{paramName}});
{{/each}}
    const snapshot = await ref.collection('{{targetStorage.collectionName}}').limit(1).get();
    return snapshot.empty ? null : {{entity}}.fromFirestore(snapshot.docs[0].id, snapshot.docs[0].data());
  }
{{else}}
  /**
   * Get {{name}} for {{../entityName}} (foreign key lookup)
   */
  async get{{capitalize name}}({{../entityNameLower}}Id: string): Promise<{{entity}} | null> {
    const snapshot = await this.db
      .collection('{{targetStorage.collectionName}}')
      .where('{{foreignKey}}', '==', {{../entityNameLower}}Id)
      .limit(1)
...
{{else}}
   ...
   * TODO: Implement relationship query (target entity not available during generation)
   */
  async get{{capitalize name}}({{../entityNameLower}}Id: string): Promise<any[]> {
    return [];
  }
{{/if}}
{{/each}}
 ...
 * Route pattern: {{#each ancestorChain}}/{{collectionName}}/:{{paramName}}{{/each}}/{{collectionName}}
 */
...
{{/each}}
    const parentPath = [{{#each ancestorChain}}'{{collectionName}}', {{paramName}}, {{/each}}'{{collectionName}}'];
   ...
   * Mount at: {{#each ancestorChain}}/{{collectionName}}/:{{paramName}}{{/each}}/{{collectionName}}
   */
  getRouter(): Router {
    return this.router;
  }
